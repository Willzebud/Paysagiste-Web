...
export const AnimatedTestimonials = ({
  ...
  const rotations = [-3, -7, 4, -5, 6]; // Définir les rotations comme un tableau fixe
          ...
          <div className="relative h-80 w-full">
            <AnimatePresence>
              {testimonials.map((testimonial, index) => (
                <motion.div
                  key={`${testimonial.src}-${index}`} // Ajout de l'index pour garantir l'unicité
                  initial={{
                    opacity: 0,
                    scale: 0.9,
                    z: -100,
                    rotate: rotations[index % rotations.length], // Utilisation des rotations fixes
                  }}
                  animate={{
